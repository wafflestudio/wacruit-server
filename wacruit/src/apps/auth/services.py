...
class AuthService:
    ...
    def login(self, username: str, password: str) -> tuple[str, str]:
        ...
        if PasswordService.verify_password(password, user.password):
            access_token = self.issue_token(user.id, 24)
            refresh_token = self.issue_token(user.id, 24 * 7)
            return (access_token, refresh_token)
        raise UserNotFoundException()

    def refresh_token(self, refresh_token: str) -> tuple[str, str]:
        user_id = self.decode_token(refresh_token)["sub"]
        ...
        access_token = self.issue_token(user.id, 24)
        new_refresh_token = self.issue_token(user.id, 24 * 7)
    ...
    def issue_token(self, user_id: int, expiration_hour: int) -> str:
        header = {"alg": "HS256"}
        payload = {
            "sub": user_id,
