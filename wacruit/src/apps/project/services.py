...
class ProjectService:
    ...
    def list_projects(self) -> ListResponse[ProjectBriefResponse]:
        ...
        for project in projects:
            items.append(
                ProjectBriefResponse.from_orm(project)
            )
        return ListResponse(items=items)
    
    def update_project(self, project_id: int, request: ProjectUpdateRequest) -> ProjectDetailResponse:
        project = self.project_repository.get_project_by_id(project_id)
        if not project:
            raise ProjectNotFoundException
    ...
    def list_project_members(self, project_id: int):
        ...
        return ListResponse(items=[
            ProjectMemberResponse.from_orm(member) for member in project.members
        ])
    
    def update_project_member(self, project_id: int, member_id: int, request: ProjectMemberUpdateRequest):
        project = self.project_repository.get_project_by_id(project_id)
        if not project:
            raise ProjectNotFoundException
