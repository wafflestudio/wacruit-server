...
class SponsorService:
    def __init__(self,
                 sponsor_repository: Annotated[SponsorRepository, Depends()]
        ):
    ...
    def get_all_sponsors(self) -> ListResponse[SponsorBriefResponse]:
        ...
        return ListResponse[SponsorBriefResponse](
            items=[SponsorBriefResponse.from_orm(sponsor) for sponsor in sponsors]
        ) if sponsors else ListResponse[SponsorBriefResponse](items=[])
    
    def update_sponsor(self, sponsor_id: str, request: SponsorUpdateRequest) -> SponsorInfoResponse:
        sponsor = self.sponsor_repository.get_sponsor_by_id(sponsor_id)
        if not sponsor:
            raise SponsorNotFoundException
